...
class InfluxDB(Store):
    ...
    def write(self, exchange, data_type, pair, timestamp):
        ...
        if data_type == TRADES:
            ...
            for entry in self.data:
                ts = int(Decimal(entry["timestamp"]) * 1000000000)
                agg.append(f'{data_type}-{exchange},pair={pair},exchange={exchange} side="{entry["side"]}",leaves_qty={entry["leaves_qty"]},order_id="{entry["order_id"]}",price="{entry["price"]}",times$
